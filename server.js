...
app.post('/api/generate', async (req, res) => {
  try {
    // 🛡️ DETEKSI EKSTRA: Biar huruf besar/kecil dari index.html tetep kebaca dan gak lolos bray!
    const prompt = req.body.prompt || req.body.Prompt;
    const apiKey = req.body.apiKey || req.body.apikey || req.body.ApiKey || req.body.APIKey;
    const modelType = req.body.modelType || req.body.model || 'hunyuan_video';
    const imageUrl = req.body.imageUrl || req.body.image;

    if (!apiKey) {
      return res.status(400).json({ error: 'API Key kosong di backend bray! Coba ketik ulang.' });
    }
